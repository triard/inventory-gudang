                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td class="td-actions text-center">
                                    ...
                                    &nbsp; -->
                                    <button type="button" onclick="ganti(<?php echo $k->id_baku;?>)" rel="tooltip" class="btn btn-success btn-round" data-original-title="" title="">
                                       <i class="zmdi zmdi-edit zmdi-hc-fw"></i>
                                    </button>
                                    &nbsp;
                                    <button type="button" rel="tooltip" class="btn btn-danger btn-round" data-original-title="" title="" onclick="hapus(<?php echo $k->id_baku;?>)">
...
$(document).ready(function () {
	setTimeout(function() {
		...
		$(".xform").on("submit", (function (b) {
			...
			var a;
			if (simpan == "tambah") {
				a = "<?php echo base_url();?>Baku/add"
			} else if (simpan == "update") {
				a = "<?php echo base_url();?>Baku/update"
			} else if (simpan == "restock") {
				a = "<?php echo base_url();?>Baku/updateStok"
			} else if (simpan == "limitstock") {
				a = "<?php echo base_url();?>Baku/updateLimitStok"
			} 
			$.ajax({
				url: a,
				type: "POST",
...
function restock() {
	...
	$(".form")[0].reset();
	$("#myModal").modal("show");
	$("#modalbody").load("<?php echo base_url();?>Baku/restock/", function (a) {
		$("#modalbody").html(a)
	})
}
...
function limitstock() {
	...
	$(".form")[0].reset();
	$("#myModal").modal("show");
	$("#modalbody").load("<?php echo base_url();?>Baku/limitstock/", function (a) {
		$("#modalbody").html(a)
	})
}
...
function hapus(a) {
	Swal.fire({
	  ...
	  if (result.value == true) {
		$.get("<?php echo base_url()?>Baku/delete/" + a, function (b) { location.reload(); })
	  }
	})
};
</script>
